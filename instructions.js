...
function UpdateInventoryAfterCustomerPurchase(){
    var updatedProductQuantity = results[0].product_stock_quantity - buyerChoice.unitsRequested;
    var query = connection.query("UPDATE productItems SET ? WHERE ?", 
        [
            {
                product_id: buyerChoice.productId
            },
            {
                product_stock_quantity: buyerChoice.unitsRequested
            }
        ],
        function(err, results){
            console.log(results)

            //logs the actualy query being run
